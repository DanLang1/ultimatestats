...
function teamStatsCSV(stats: AdvancedTeamStats, teamName: string): string {
  return [
    ...
    csvRow(['Scores After Turnover', stats.scoresAfterTurnovers, '']),
    csvRow(['Offensive Points', stats.oPoints, '']),
    csvRow(['Defensive Points', stats.dPoints, '']),
    csvRow(['Total Turnovers', stats.totalTurnovers, '']),
    csvRow(['Total Blocks', stats.totalBlocks, '']),
    csvRow(['Turnovers per Point', formatDecimal(stats.turnoversPerPoint ?? 0), '']),
    csvRow(['Points per Turnover', formatDecimal(stats.pointsPerTurnover ?? 0), '']),
    csvRow(['Blocks per D-Point', formatDecimal(stats.blocksPerDPoint), '']),
    csvRow(['Possessions per Point', formatDecimal(stats.possessionsPerPoint ?? 0), '']),
