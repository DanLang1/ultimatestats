...
function teamStatsCSV(stats: TeamStats): string {
  ...
  return [
    ...
    csvRow(['Total Turns', stats.totalTurnovers, 'Our turnovers']),
    csvRow(['Opponent Turns', stats.opponentTurnovers, 'Gives us possession']),
    csvRow(['Turns per Point', formatDecimal(stats.turnoversPerPoint), '']),
    csvRow(['Turns per O-Point', formatDecimal(stats.turnoversPerOPoint), '']),
    csvRow(['Points per Turn', formatDecimal(stats.pointsPerTurnover), '']),
...
export const formatDate = formatTimestampForDisplay;
export const formatDateForCSV = formatTimestampForCSV;
