...
1. `AGENTS.md` and current domain documentation for intended contracts
2. Exported types and tests for current implemented behavior
...
If intended contracts and implemented behavior disagree, flag and reconcile the inconsistency
instead of silently choosing one source. Do not copy an advanced implementation that conflicts with
a newer `AGENTS.md` rule or domain document.
...
| Behavior verification       | `test/routes/AdvancedRoutes.test.tsx`, `test/routes/AdvancedAnalyticsRoutes.test.tsx`, `components/advancedTracking/__tests__/`, `.maestro/tests/` | Focused route/component tests plus deterministic device flows                                        |
